...
def test_process_sales_text_parsing(mock_openai_class):
    ...
    result = processor.process_sales_text(sales_text)
    print(f"DEBUG Result: {result}")

    # Assertions
    assert result['transaction_type'] == 'Sale'
    assert result['total_amount'] == 3360.00
    assert result['vendor_name'] == 'Apollo Pharm'

def test_mock_ledger_row_conversion():
    # Sample AI Output
    ai_output = {
        "invoice_no": "INV-202",
        "date": "23-02-2026",
